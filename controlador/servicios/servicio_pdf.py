...
def generar_contrato_pdf(datos_contrato: dict) -> str:
    ...
    contenido = contenido.replace("[NUMERO_FINCA]", finca.get("numero_finca", "________"))
    contenido = contenido.replace("[METROS_CONSTRUIDOS]", finca.get("metros_construidos", "____"))
    contenido = contenido.replace("[SUPERFICIE_TOTAL]", finca.get("superficie_total", "____"))
    contenido = contenido.replace("[FECHA_LIMITE]", fechas.get("limite", "___/___/______"))

    # 4b. Reemplazar campos extra (alquiler y compraventa)
    extra = datos_contrato.get("extra", {})
    contenido = contenido.replace("[PAGADOR_IBI_COMUNIDAD]", extra.get("pagador_ibi", "ARRENDADOR"))
    contenido = contenido.replace("[PAGADOR_LUZ]", extra.get("pagador_luz", "ARRENDATARIO"))
    contenido = contenido.replace("[PAGADOR_AGUA]", extra.get("pagador_agua", "ARRENDATARIO"))
    contenido = contenido.replace("[PAGADOR_GAS]", extra.get("pagador_gas", "ARRENDATARIO"))
    contenido = contenido.replace("[DURACION_ANOS]", extra.get("duracion_anos", "__"))
    contenido = contenido.replace("[FECHA_ENTRADA]", extra.get("fecha_entrada", "___/___/______"))
    contenido = contenido.replace("[RENTA_ANUAL]", extra.get("renta_anual", "________"))
    contenido = contenido.replace("[RENTA_MENSUAL]", extra.get("renta_mensual", "________"))
    contenido = contenido.replace("[IBAN_ARRENDADOR]", extra.get("iban_arrendador", "________________________________"))
    contenido = contenido.replace("[FIANZA]", extra.get("fianza_euros", "________"))
    contenido = contenido.replace("[DESCRIPCION_CARGA]", extra.get("descripcion_carga", "________________"))
    contenido = contenido.replace("[RESTO_ESCRITURA]", extra.get("resto_escritura", "________"))
    contenido = contenido.replace("[CIUDAD]", finca.get("ciudad", "________________"))
    contenido = contenido.replace("[FECHA_CONTRATO]", fechas.get("firma", "___/___/______"))
    contenido = contenido.replace("[REFERENCIA_CATASTRAL]", finca.get("referencia_catastral", "________________"))
    contenido = contenido.replace("[DIRECCION_VIVIENDA]", finca.get("direccion", "________________"))
